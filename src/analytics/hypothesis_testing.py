...
def run_hypothesis_test():
    ...
    # Filtra os grupos: COM Call2Go (link_direto + texto_implicito) vs SEM
    group_none = df[df['call2go_type'] == 'nenhum']['view_count']
    group_call2go = df[df['call2go_type'] != 'nenhum']['view_count']

    print(f"Tamanho do Grupo de Controle (Sem Call2Go): {len(group_none)}")
    print(
        f"Tamanho do Grupo de Tratamento (Com Call2Go): {len(group_call2go)}")
    ...
    if p_value < alpha:
        ...
        print(">> Há evidências estatísticas significativas (com 95% de confiança) de que vídeos COM Call2Go geram MAIOR volume de visualizações do que vídeos sem a chamada.")
    else:
