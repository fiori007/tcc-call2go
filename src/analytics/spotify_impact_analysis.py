...
def run_spotify_impact_test():
    ...
    plt.xscale('log')
    plt.title("Correlação entre Views no YouTube e Popularidade no Spotify", fontsize=14, fontweight='bold')
    ...
    print(f"✅ Gráfico de dispersão salvo para o TCC em: {plot_path}\n")
    
    # ---------------------------------------------------------
    # 2. Teste Estatístico (Popularidade no Spotify)
    # ---------------------------------------------------------
    group_none = df[df['call2go_type'] == 'nenhum']['popularity']
    group_implicit = df[df['call2go_type'] == 'texto_implicito']['popularity']
    
    print("--- ESTATÍSTICA DESCRITIVA: POPULARIDADE NO SPOTIFY ---")
    print(f"Média Pop (Sem Chamada): {group_none.mean():.2f}")
    print(f"Média Pop (Texto Implícito): {group_implicit.mean():.2f}\n")
    
    print("--- RESULTADO DO TESTE DE HIPÓTESE (SPOTIFY) ---")
    # Testa se a popularidade é maior nos casos em que houve Call2Go implícito
    # Usamos t-test de amostras independentes se a variância permitir, ou Mann-Whitney novamente
