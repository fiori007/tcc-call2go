...
\title{Detecção Automatizada de Estratégias \textit{Call2Go} em Vídeos do YouTube:\\
       Um Pipeline Reprodutível para Análise \textit{Cross-Platform}\\
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw=black!70, fill=gray!10, text width=8.8cm,
                text centered, minimum height=0.58cm, font=\small,
                rounded corners=2pt},
    abox/.style={rectangle, draw=green!60!black, fill=green!8, text width=8.8cm,
                 text centered, minimum height=0.58cm, font=\small,
                 rounded corners=2pt},
    vbox/.style={rectangle, draw=blue!60, fill=blue!8, text width=8.8cm,
                 text centered, minimum height=0.58cm, font=\small,
                 rounded corners=2pt},
    arr/.style={-{Stealth}, thick, draw=black!60}
]
\node[box]  (s1) {Etapas 1--5: Coleta (Artistas, YouTube, Spotify, Last.fm, \textit{Scraping} Canais)};
...
Os resultados indicam interpretação diferenciada por nível. \textbf{Nível
canal}: $\kappa = 0{,}80$ (substancial) e acurácia de 90,1\% rejeitam $H_0$,
validando o detector nesse nível como metodologicamente confiável. O
desempenho superior é esperado: o canal concentra a evidência principal do
Call2Go, e o scraping da aba ``Sobre'' captura os links de forma estruturada.
\textbf{Nível vídeo}: $\kappa = 0{,}45$ (moderado) e acurácia de 82,4\% não
rejeitam $H_0$, indicando que o detector restrito às descrições individuais
não atinge o limiar de 85\%, resultado metodologicamente relevante: artistas
frequentemente inserem o Call2Go apenas no perfil do canal, não em cada
descrição de vídeo.

\begin{figure}[H]
\centering
\includegraphics[width=.78\textwidth]{figs/validation_metrics_per_class.png}
\caption{Métricas de validação (precisão, \textit{recall}, F1-score) por
classe e por nível de análise. O nível canal apresenta desempenho
substancialmente superior ao nível vídeo isolado.}
\label{fig:val_metrics}
\end{figure}

O nível vídeo sofre especialmente em \textit{recall} para
\texttt{link\_direto}---vídeos cujo Call2Go está apenas no canal. A métrica
OR, ao agregar os pontos fortes de ambos os níveis, representa o modo de
operação recomendado do sistema.
...
O presente trabalho foi realizado com apoio da Pontifícia Universidade
Católica de Minas Gerais. Ferramentas de Inteligência Artificial (GitHub
Copilot) foram utilizadas como apoio na revisão e na formatação de trechos
do código do pipeline ETL e do presente documento, em conformidade com o
Art.~$6^{\circ}$ do Regulamento Geral de TCC 2025.
